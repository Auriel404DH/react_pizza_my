...
import PizzaCart from './../pizzaBlock/pizzaCart';
import {
  clearCart,
  removeCartPizza,
  plusCartPizza,
...
function Cart() {
  ...
  const { items, totalCount, totalPrice } = useSelector(({ cartReduser }) => cartReduser);
  ...
  const onClearCart = () => {
    if (window.confirm('Ты уверен? (Подумой)')) {
      dispatch(clearCart());
    }
  };

  const onDeleteItem = (id) => {
    if (window.confirm('Ты уверен на счот пицы? (Подумой)')) {
